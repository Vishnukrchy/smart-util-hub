...
const generateInstagramPost = async (prompt: string): Promise<string> => {
  // TODO: Replace with actual Gemini API call
  // Example: await fetch('/api/gemini/instagram', { ... })
  return `Generated Instagram Post for: ${prompt}`;
};
...
const InstagramPostGenerator: React.FC = () => {
  ...
  const handleGenerate = async () => {
    setLoading(true);
    setResult('');
    const post = await generateInstagramPost(prompt);
    setResult(post);
    setLoading(false);
  };
  ...
  return (
    <div className="max-w-xl mx-auto p-6">
      <h1 className="text-2xl font-bold mb-4">Instagram Post Generator</h1>
      <textarea
